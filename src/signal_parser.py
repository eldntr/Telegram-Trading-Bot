...
class TelegramSignalParser:
    """Parses Telegram messages into structured data models."""

    def _extract_common_attributes(self, message_obj: Any) -> dict:
        """Extracts common attributes from a Telethon message object."""
        return {
    ...
    def parse_message(self, message_obj: Any) -> BaseMessage:
        """Analyzes a Telethon message object and returns a structured data model."""
        if not message_obj or not getattr(message_obj, 'raw_text', None):
            # Gracefully handle messages with no text content (e.g., photos, stickers)
            return UnstructuredMessage(
                raw_text="[Media or Action message: No text content]",
                timestamp=getattr(message_obj, 'date', datetime.now()),
                sender_id=getattr(message_obj, 'sender_id', None),
                message_id=getattr(message_obj, 'id', None),
                content="[Media or Action message: No text content]",
    ...
    def _try_parse_daily_recap(self, first_line, text, lines, common_attrs):
        ...
        if "DAILY RECAP" in first_line:
            ...
            patterns = {
                "target_1": r"✅ Hitted target 1:\s*(.*)",
                "target_2": r"✅ Hitted target 2:\s*(.*)",
                "target_3": r"✅ Hitted target 3:\s*(.*)",
                "target_4": r"✅ Hitted target 4:\s*(.*)",
            }
            for line in lines:
            ...
            if total_match := re.search(r"Total Signals:\s*(\d+)", text):
                recap.total_signals = int(total_match.group(1))
            if tp_match := re.search(r"Hitted Take-Profits:\s*(\d+)", text):
                recap.total_take_profits = int(tp_match.group(1))
            if sl_match := re.search(r"Hitted Stop-Losses:\s*(\d+)", text):
                recap.total_stop_losses = int(sl_match.group(1))
            return recap
        return None
    ...
    def _try_parse_new_signal_alert(self, first_line, text, lines, common_attrs):
        if first_line == "🆕 NEW SIGNAL 🆕":
            pattern = re.compile(
                r"(?:❗❗❗|⚡⚡⚡)\s*([A-Z]+)\s*price\s*(?:amplitude is|decreased)\s*([+-]?[\d.]+)%\s*in the last\s*(\d+)\s*minutes"
            )
            ...
            if match:
                return MarketAlert(
                    **common_attrs,
                    coin=match.group(1),
                    price_change_percentage=float(match.group(2)),
                    timeframe_minutes=int(match.group(3)),
                    alert_message=text.strip()
                )
        return None
    ...
    def _try_parse_new_signal(self, first_line, text, lines, common_attrs):
        if match := re.match(r"🆕\s*NEW SIGNAL:\s*([A-Z0-9]+USDT)\s*🆕", first_line):
            ...
            risk_level_match = re.search(r"Risk Level:\s*(?:🟢|⚠️)\s*(\w+)", text)
            risk_level = risk_level_match.group(1) if risk_level_match else None
            entry_price_match = re.search(r"Entry:\s*([\d.]+)", text)
            entry_price = float(entry_price_match.group(1)) if entry_price_match else None
            targets, stop_losses = [], []
            try:
                separator = "----------------------------------------------"
                indices = [i for i, line in enumerate(lines) if line.strip() == separator]
                if len(indices) >= 3:
                    data_start = indices[1] + 1
                    data_end = indices[2]
                    targets, stop_losses = self._parse_targets_and_sl_from_new_signal(lines[data_start:data_end])
            except (ValueError, IndexError):
                pass
            social_link_match = re.search(r"Cek Sentimen Sosial Media untuk \$?\S+", text)
            social_link = social_link_match.group(0) if social_link_match else None
            data_link_match = re.search(r"Analisis Data Coinglass untuk \S+", text)
            data_link = data_link_match.group(0) if data_link_match else None
            return NewSignal(
                **common_attrs, coin_pair=match.group(1), risk_rank=risk_rank,
                risk_level=risk_level, entry_price=entry_price, targets=targets,
                stop_losses=stop_losses, social_media_link=social_link,
    ...
    def _try_parse_signal_update(self, first_line, text, lines, common_attrs):
        if match := re.match(r"^(✅|🔴)\s*SIGNAL UPDATE:\s*([A-Z0-9]+USDT)\s*(✅|🔴)", first_line):
            ...
            update_type = "TARGET_HIT" if targets_hit else "STOP_LOSS_TRIGGERED"
            return SignalUpdate(**common_attrs, coin_pair=match.group(2), targets_hit=targets_hit, stop_losses_triggered=sl_triggered, update_type=update_type)
        return None
